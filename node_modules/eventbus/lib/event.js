 ...
 * @private
 */
exports = module.exports = ovy.define({
    ...
    constructor: function(eventbus, name) {
        this.name = name;
    ...
    addListener: function(fn, scope, options) {
        var me = this,
            listener;
        scope = scope || me.eventbus;

        //<debug error>
        if (!fn) {
            throw new Error("The specified callback function is undefined");
        }
        //</debug>
    ...
    findListener: function(fn, scope) {
        ...
        while (i--) {
            ...
            if (listener) {
                ...
                // We cannot use the test (s == scope || s == this.eventbus)
                // Otherwise, if the Observable itself adds ovy.emptyFn as a listener, and then ovy.emptyFn is added under another scope, there will be a false match.
                if (listener.fn == fn && (s == (scope || this.eventbus))) {
                    return i;
                }
            }
        }
    ...
    removeListener: function(fn, scope) {
        ...
        if (index != -1) {
            ...
            ovy.arrays.erase(me.listeners, index, 1);
            return true;
        }
    ...
    fire: function() {
        ...
        if (count > 0) {
            ...
            for (i = 0; i < count; i++) {
                ...
                if (listener && listener.fireFn.apply(listener.scope || me.eventbus, args) === false) {
    ...
    createBuffered: function (handler, listener, o, scope) {
        listener.task = new ovy.utils.DelayedTask();
        return function() {
            listener.task.delay(o.buffer, handler, scope, ovy.arrays.toArray(arguments));
        };
    },

    createDelayed: function (handler, listener, o, scope) {
        return function() {
            var task = new ovy.utils.DelayedTask();
            if (!listener.tasks) {
                listener.tasks = [];
            }
            listener.tasks.push(task);
            task.delay(o.delay || 10, handler, scope, ovy.arrays.toArray(arguments));
        };
    },
    ...
    createSingle: function (handler, listener, o, scope) {
        return function() {
            ...
            if (event.removeListener(listener.fn, scope) && event.eventbus) {
                ...
                event.eventbus.hasListeners[event.name]--;
            }
